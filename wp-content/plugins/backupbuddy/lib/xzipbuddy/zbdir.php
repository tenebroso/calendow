...
if ( !class_exists( "pluginbuddy_zbdir" ) ) {
	...
	abstract class pluginbuddy_zbdir_xclusion {
		 ...
		 *
		 */
		public function __construct( array $unknowns = array(), array $opts = array() ) {
		 ...
		 *
		 */
		public function add_pattern( array $xclusions = array() ) {
		...
		public function matches_regex( $path = '' ) {
			...
			if ( is_string( $path ) ) {
				...
				foreach ( $this->_patterns as $pattern ) {
					...
					$escaped_pattern = str_replace( self::DELIMITER, self::ESCAPED_DELIMITER, $pattern );
					
					if ( 1 === preg_match( self::DELIMITER . $escaped_pattern . self::DELIMITER, $path ) ) {
	...
	class pluginbuddy_zbdir_visitor_details extends pluginbuddy_zbdir_visitor {
		...
		// Set options for what information this handler wants when items
		// are added - e.g., just file name or maybe name and size, etc.
		public function __construct() {
		...
		public function update( $key, $updates = array() ) {
			...
			if ( isset( $this->_items[ $key ] ) ) {
				...
				$this->_items[ $key ] = array_merge( $this->_items[ $key ], $updates );
	...
	class pluginbuddy_zbdir {
		...
		public function __construct( $root = '', $options = array() ) {
			...
			// Now create our exclusion/inclusion handlers and populate them.
			$this->_exclusions_handler = new pluginbuddy_zbdir_exclusion( $this->_options[ 'exclusions' ] );
			$this->_exclusions_handler->add_pattern( $this->_options[ 'pattern_exclusions' ] );
			$this->_inclusions_handler = new pluginbuddy_zbdir_inclusion( $this->_options[ 'inclusions' ] );
		...
		public function __destruct( ) {
			...
			// Destroy our exclusions handler
			unset( $this->_exclusions_handler );
			
			// Destroy our inclusions handler
