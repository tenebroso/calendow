						...
						<?php $the_query = new WP_Query( $args ); 
								if ( $the_query->have_posts() ) : 
									while ( $the_query->have_posts() ) : 
										...
										$img = get_field('read_this_thumbnail_image'); 
										$customLink = get_field('custom_url');
										if($x != 4): ?><li>

								<?php if($img): ?>
								<a class="center-block center-block-bg-img" href="<?php if($customLink): echo $customLink; else: the_permalink(); endif; ?>" style="background-image:url(<?php echo $img; ?>);"></a>
								<?php else: ?>
								<a class="center-block" href="<?php if($customLink): echo $customLink; else: the_permalink(); endif; ?>">
